              ...
              <div class="p-4">
                <form wire:submit.prevent="submit" class="space-y-4">
                    <div>
                        ...
                        <input type="date" wire:model="date" class="border-gray-300 rounded-lg w-full p-2" required>
                        @error('date') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
        
                    <div>
                        <label for="amount" class="block mb-2 text-sm font-medium text-gray-900">Amount</label>
                        <input type="number" wire:model="amount" class="border-gray-300 rounded-lg w-full p-2" required>
                        @error('amount') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
        
                    <div>
                        <label for="order" class="block mb-2 text-sm font-medium text-gray-900">Payment Method</label>
                        <select wire:model="payment" class="border-gray-300 rounded-lg w-full p-2" required>
                            <option value="">Select payment</option>
                            @foreach ($payments as $payment)
                                <option value="{{ $payment->id }}">{{ strtoupper($payment->name) }}</option>
                            @endforeach
                        </select>
                        @error('order') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
        
                    <button type="submit" class="w-full bg-blue-700 text-white p-2 rounded-lg">Submit</button>
                </form>
            </div>
        </div>
    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
        <div class="w-full mb-1">
            <div class="mb-4">
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                      ...
                      <li>
                        <div class="flex items-center">
                          ...
                          <a href="#" class="ml-1 text-gray-700 hover:text-primary-600 md:ml-2 dark:text-gray-300 dark:hover:text-white">Admins</a>
                        </div>
                      </li>
                      <li>
                        <div class="flex items-center">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">All Admins</h1>
            </div>
            <div class="sm:flex">
                            ...
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap">
                                    ...
                                    <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                        <div class="text-base font-semibold text-gray-900 dark:text-white">{{number_format($balance->amount)}}</div>
                                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">{{$balance->created_at->diffForHumans()}}
                                    </div>
                                </td>
                                <td class="max-w-sm p-4 overflow-hidden text-base font-normal uppercase text-gray-500 truncate xl:max-w-xs dark:text-gray-400">{{$balance->payment->name}}</td>
                                <td class="max-w-sm p-4 overflow-hidden text-base font-normal uppercase text-gray-500 truncate xl:max-w-xs dark:text-gray-400">{{$balance->date}}</td>
